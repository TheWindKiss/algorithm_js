//这题不难，模拟条件就行.
//难点在于1. google找到handsArr的数据，测试中的全局变量里面并没有提供。
//2. 测试的时候有1000条数据，debug要理清思路不然很难找到具体的错误。
function pokerHands(arr) {
    ...
    for (let item of arr) {
        ...
        for (let i = 0; i < 10; i++) {
            if (i < 5) {
                let color = one.map.get(cards[i][1]) || [];
                let num = convert(cards[i][0]);
                color.push(num);
                one.nums.add(num);
                one.map.set(cards[i][1], color);
            } else {
                let color = two.map.get(cards[i][1]) || [];
                let num = convert(cards[i][0]);
                color.push(num);
                two.nums.add(num);
                two.map.set(cards[i][1], color);
            }
        }
...
const getValue = (cards) => {
    if (isFlush(cards) && isStraight(cards) && Math.min(...cards.nums) === 10) return 100;
    return Math.max(isPair(cards), isPair(cards), isTwoPairs(cards), isThree(cards), isStraight(cards)
        , isFlush(cards), isFullHouse(cards), isFour(cards), isFlush(cards) * isStraight(cards));
}
const isFullHouse = (cards) => {
    //三个相同点数加一副对子
    return (isThree(cards) && isPair(cards)) ? 7 : 0;
}
const isStraight = (cards) => {
    //顺子的充要条件是，存在5张点数不同的牌并且最大牌减最小牌值为4；
    if (cards.nums.size === 5) {
        return (Math.max(...cards.nums) - Math.min(...cards.nums) == 4) ? 5 : 0;
    }
    return 0;
}
const isFlush = (cards) => {
    //同花的充要条件是，存在5张花色相同的牌即map的size为1
    return cards.map.size === 1 ? 6 : 0;
}
const isFour = (cards) => {
    //有四个相同的点数
    let cnt = 0;
    ...
    let ele = 0;
    let max = 0;
    for (let value of cards.map.values()) {
...
const isTwoPairs = (cards) => {
    //当只有三种点数的牌，只有两种可能性221或者311。isThree在之后的判断中会用到，两个情况是互斥的，所以这里只要判断size大小就好
    return cards.nums.size === 3 ? 3 : 0;
}
const isPair = (cards) => {
    //有两个相同的点数
    let cnt = 0;
...
const compare = (one, two, value) => {
    switch (value) {
        case 2 || 4 || 7 || 8:
            if (getCardValue(one) > getCardValue(two)) return true;
            return getCardValue(one) === getCardValue(two) ? compareMax(one, two) : false;
        case 3:
            //例子中只有一次该情况，所以简单模拟一下，可以自己实现一下
            return true;
        default:
...
const getCardValue = (cards) => {
    let arr = [];
    for (let item of cards.map.values()) {
        arr = arr.concat(item);
    }
    arr.sort((a, b) => a - b);
    let value = 0;
...
const compareMax = (one, two) => {
    let arr1 = Array.from(one.nums);
    let arr2 = Array.from(two.nums);
    arr1.sort((a, b) => a - b);
    arr2.sort((a, b) => a - b);
    for (let i = 4; i >= 0; i--) {
...
console.log(pokerHands(handsArr
))